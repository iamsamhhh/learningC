...
int main()
{
    GLFWwindow* window = creatWindow(SCR_WIDTH, SCR_HEIGHT);
    if (window == NULL) {
        std::cout << "Failed to creat a window" << std::endl;
        glfwTerminate();
        return -1;
    }
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(house_body), house_body, GL_STATIC_DRAW);
